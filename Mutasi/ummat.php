...
<body>
        ...
        $Akun  = "SELECT * FROM nasabah WHERE produk = 'Tab.Ummat'";
        $rAkun = mysqli_query($koneksi, $Akun);
        while ($dAkun = mysqli_fetch_assoc($rAkun)){
        ?>
    <div class="mt-3 card">
        <div class="card-header text-white bg-success">
            <span>No. Rekening = <?= noRek('Tab.Ummat') . $dAkun['nomer'] ?> ||</span>
        ...
        <div class="container">
            <table class="table table-bordered">
                ...
                <?php
                    ...
                    $totalKredit= 0;
                    $nama       = $dAkun['nama'];
                    $query      = "SELECT * FROM transaksi WHERE nama = '$nama' && produk = 'Tab.Ummat' ORDER BY id ASC";
